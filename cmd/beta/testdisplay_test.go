...
func TestDisplayLayout(t *testing.T) {
	...
	// Test the spacing
	assert.Equal(t, 123, len(lines[1]), "Each line should be 123 characters long")
	for i := 2; i <= 6; i++ {
		assert.Equal(t, 123, len(lines[i]), "Each line should be 123 characters long")
	}

	// Test the borders
	assert.Equal(t, '┌', rune(lines[0][0]))
	assert.Equal(t, '┐', rune(lines[0][len(lines[0])-1]))
	assert.Equal(t, '└', rune(lines[8][0]))
	assert.Equal(t, '┘', rune(lines[8][len(lines[8])-1]))

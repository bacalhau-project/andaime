...
func (c *LiveGCPClient) EnsureProject(ctx context.Context, projectID string) (string, error) {
	...
	if m.Deployment.GCP.OrganizationID == "" {
...
func (c *LiveGCPClient) EnsureServiceAccountSetup(
	ctx context.Context,
	projectID string,
	...
	if err := c.ensureRequiredAPIsEnabled(ctx, projectID); err != nil {
		return nil, fmt.Errorf("failed to enable required APIs: %v", err)
	}

	// Try to get or create the service account
	sa, err := c.getOrCreateServiceAccount(ctx, projectID, serviceAccountEmail)
	if err != nil {
		return nil, err
	}

	// Ensure the service account has the necessary roles
	if err := c.ensureServiceAccountRoles(ctx, projectID, sa.Email); err != nil {
		return nil, fmt.Errorf("failed to ensure service account roles: %v", err)
	}

	return sa, nil
}

func (c *LiveGCPClient) ensureRequiredAPIsEnabled(ctx context.Context, projectID string) error {
	requiredAPIs := []string{
		"iam.googleapis.com",
		"cloudresourcemanager.googleapis.com",
		"compute.googleapis.com",
...
func (c *LiveGCPClient) enableAPI(ctx context.Context, projectID, apiName string) error {
	...
	_, err := c.serviceUsageService.Services.Enable(name, &serviceusage.EnableServiceRequest{}).
		Context(ctx).
		Do()
	return err
}
...
func (c *LiveGCPClient) getOrCreateServiceAccount(
	...
	sa, err := c.iamService.Projects.ServiceAccounts.Get(
		fmt.Sprintf(
...
func (c *LiveGCPClient) grantRoleToServiceAccount(
	...
	for _, binding := range policy.Bindings {
		if binding.Role == role {
			...
			_, err := c.resourceManagerService.Projects.SetIamPolicy(projectID, &cloudresourcemanager.SetIamPolicyRequest{Policy: policy}).
				Context(ctx).
				Do()
			return err
		}
	...
	// If we're here, the role binding doesn't exist yet
...
func (c *LiveGCPClient) testProjectPermissions(
	ctx context.Context,
	...
	return backoff.Retry(func() error {
		...
		_, err = c.serviceUsageClient.BatchGetServices(ctx, servicesRequest)
		if err != nil {
			return fmt.Errorf(
...
func (c *LiveGCPClient) SetBillingAccount(
	...
	return backoff.Retry(func() error {
		...
		serviceAccount, err := c.ensureServiceAccount(ctx, projectID)
		if err != nil {
			l.Warnf("Failed to ensure service account: %v. Retrying...", err)
			return err
		}

		// Then, set the billing account
		...
		l.Infof("Successfully set billing account %s for project %s using service account %s",
			billingAccountID, projectID, serviceAccount.Email)
		return nil
	}, b)
}

func (c *LiveGCPClient) grantBillingRoles(
	ctx context.Context,
	projectID, serviceAccountEmail string,
) error {
	l := logger.Get()
	...
	for _, role := range roles {
		...
		_, err = c.resourceManagerService.Projects.SetIamPolicy(projectID, &cloudresourcemanager.SetIamPolicyRequest{
			...
			Do()
		if err != nil {
			return fmt.Errorf("failed to set IAM policy for role %s: %v", role, err)
		}
		l.Infof("Granted role %s to service account %s", role, serviceAccountEmail)
	}
